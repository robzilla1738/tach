...
# driven entirely through the `tach` binary — the path an external coding agent
...
#   - `tach init --existing` adopts a real repo (Tachfile/TACH_AGENT.md/.tachignore),
#     detects the test command, scaffolds no toy source, and never touches AGENTS.md;
#     TACH_AGENT.md states commit is ledger-only (not git).
#   - `tach guard begin` opens a coding session with a durable baseline.
#   - `tach guard context --json` is a usable operating contract.
#   - an in-scope fix verifies, mints a shell.run receipt with captured artifacts,
#     and `tach guard commit` finalizes Tach's ledger only — git is untouched.
...
#   - `tach goal replay` is consistent, and `--rerun` reproduces the verdict.
#
...
echo 'user-owned — tach must never touch this' > AGENTS.md
...
# ---------------------------------------------------------------------------
echo "## tach init --existing  (adopt the repo; detect; write 3 files; leave source + AGENTS.md alone)"
"$BIN" init --existing >/dev/null
for f in Tachfile TACH_AGENT.md .tachignore; do
  [ -f "$f" ] || { echo "FAIL: init --existing did not write $f"; exit 1; }
done
[ -f src/main.tach ] && { echo "FAIL: init scaffolded toy source"; exit 1; }
grep -q 'cargo test' Tachfile || { echo "FAIL: Tachfile did not detect 'cargo test'"; exit 1; }
grep -q 'command("cargo test").passes' Tachfile || { echo "FAIL: Tachfile missing command(...).passes require"; exit 1; }
[ "$(cat AGENTS.md)" = 'user-owned — tach must never touch this' ] || { echo "FAIL: AGENTS.md was modified"; exit 1; }
grep -qi 'ledger' TACH_AGENT.md || { echo "FAIL: TACH_AGENT.md does not explain ledger-only commit"; exit 1; }
...
EOF
"$BIN" check Tachfile >/dev/null 2>&1 || true   # Tachfile isn't a .tach module; begin re-validates it

# ---------------------------------------------------------------------------
echo "## tach guard begin  (open a coding session over the broken tree)"
RUN_ID="$("$BIN" guard begin FixFailingTests --json | jget '["run_id"]')"
echo "## run id: $RUN_ID"
[ -n "$RUN_ID" ] || { echo "FAIL: no run id from guard begin"; exit 1; }
KIND="$(python3 -c 'import json;print(json.load(open(".tach/goals/'"$RUN_ID"'/state.json"))["kind"])')"
[ "$KIND" = coding ] || { echo "FAIL: state.json kind=$KIND, expected coding"; exit 1; }
[ -f ".tach/goals/$RUN_ID/baseline.json" ] || { echo "FAIL: no baseline.json"; exit 1; }
echo "   ok — coding session open with a durable baseline"

echo "## tach guard context --json  (the operating contract is usable)"
...
RCPTS="$(ls -1 ".tach/goals/$RUN_ID/receipts/"*.json 2>/dev/null | wc -l | tr -d ' ')"
...
echo "## tach guard verify  (resume: the receipt is REUSED, the command does not re-run)"
...
EXEC="$(grep -c '"shell.executed"' ".tach/goals/$RUN_ID/events.jsonl" || true)"
[ "$EXEC" -eq 1 ] || { echo "FAIL: command executed $EXEC times across crash+resume (want exactly 1)"; exit 1; }
grep -q '"receipt.reused"' ".tach/goals/$RUN_ID/events.jsonl" || { echo "FAIL: no receipt.reused event on resume"; exit 1; }
# The receipt records a real captured subprocess: exit 0 + an on-disk stdout artifact.
RC="$(ls -1 ".tach/goals/$RUN_ID/receipts/"*.json | head -1)"
...
ls -1 ".tach/goals/$RUN_ID/artifacts/"*.stdout >/dev/null 2>&1 || { echo "FAIL: no stdout artifact captured"; exit 1; }
echo "   ok — command ran exactly once; receipt reused; artifacts captured"

echo "## tach guard finalize  (preferred spelling; finalize Tach's ledger ONLY — git untouched)"
"$BIN" guard finalize >/dev/null
STATUS="$(python3 -c 'import json;print(json.load(open(".tach/goals/'"$RUN_ID"'/state.json"))["status"])')"
...
grep -q '"guard.committed"' ".tach/goals/$RUN_ID/events.jsonl" || { echo "FAIL: no guard.committed event"; exit 1; }
echo "   ok — run committed into the ledger; no git repo created"

echo "## tach goal replay $RUN_ID  (consistent from receipts) + --rerun (reproduces)"
...
grep -q '"scope.violation"' ".tach/goals/$RUN2/events.jsonl" || { echo "FAIL: no scope.violation recorded"; exit 1; }
