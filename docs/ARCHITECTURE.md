...
| `runtime` | the durable executor — `step_once`/`drive` (the repair loop), the action layer's `drive_actions` (a fixed plan with approval gates and receipts), and the plan language's `drive_plan` (durable re-execution of a `plan` block); `resolve_plan` re-parses a user goal's frozen source snapshot on resume (catalog for built-ins); `resume_run`/`replay_run` dispatch on `GoalRecord.kind`. Plan replay is **replay-from-receipts**: fake tools re-invoke (and must agree), real tools are NEVER re-invoked — recorded receipt outputs feed back into the walk, a crashed run replays to its exact `frontier`, and the event hash chain is verified as part of `identical` |
...
| `tool` | tool dispatch: classifies fake vs **real** tools, authorizes real calls against the goal's grants (exact-match commands, repo-relative `cwd`, bounded timeouts), and routes invocation — `shell.run` through `shell`, `http.*` through `http`. Real outputs are receipt evidence; `tool.completed` events carry only their hash |
| `http` | the only network code in the binary: strict URL parsing (userinfo/fragments/non-ASCII hosts rejected), URL-glob authority (`url_allowed`, https unless the glob grants `http://`, redirects never followed), `headers_env` secret indirection (values never serialized), `Idempotency-Key` on every POST, bodies streamed to artifacts. Synchronous ureq + rustls — no async runtime, no system OpenSSL |
